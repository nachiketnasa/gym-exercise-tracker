...
listed here (tasks 23+) with their issue number in the heading. Once an issue is
...
## 2. Set up local PostgreSQL via Docker Compose — done (#2)
...
fetch PRs for a given exercise. Per the resolved decision in `plan.md`, PRs track
...
## 24. Edit and delete custom exercises via the Exercise API (#26)
Goal: Custom exercises can be renamed and removed through the API.
Description: Add PATCH/DELETE for custom exercises with 403 on presets, 409 on
name collision or when the exercise is referenced by entries, and 404/422 cases.
Split out of task 7 during grooming.

## 25. Multi-user authentication and request-scoped current user (#27)
Goal: Real auth replaces the single-user stub.
Description: Add authentication and resolve the "current user" per request,
replacing the seeded local user from task 14. Filed during grooming of tasks
14–17; deliberately deferred per `plan.md` roadmap.

## 26. Backend generic exception handler and production error hardening (#28)
Goal: Unhandled errors return a consistent, safe JSON shape in production.
Description: Add a catch-all exception handler and production error hardening on
top of the validation/not-found handlers from task 15. Split out of task 15
during grooming.

## 27. Compute goal attainment (met / not-met) status (#29)
Goal: Each goal reports whether it has been achieved.
Description: Derive met/not-met status for a goal by comparing its target against
the corresponding PR/logged data. Filed during grooming of tasks 11–12.

## 28. PR roll-up endpoint across all exercises (#30)
Goal: One endpoint returns PRs for every exercise at once.
Description: Add an aggregate endpoint returning current PRs across all exercises,
building on the per-exercise calculation in task 11. Filed during grooming of
task 11.
